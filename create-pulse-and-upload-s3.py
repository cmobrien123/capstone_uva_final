...
env_path = '/Users/colinobrien/Desktop'
...
random.seed(100)
train_idx = random.sample(pulses.keys(), int(np.floor(len(pulses)*0.66))) # use 2/3 as training
train = {k : pulses[k] for k in train_idx}

test_idx = set(set(pulses.keys())).difference(train_idx)
test = {k : pulses[k] for k in test_idx}

# split training into two files for ease of saving
train1 = dict(list(train.items())[len(train)//2:])
train2 = dict(list(train.items())[:len(train)//2])

# write to pickle files
with open('../data-cleaned/train1.pickle', 'wb') as f:
    pickle.dump(train1, f)

with open('../data-cleaned/train2.pickle', 'wb') as f:
    pickle.dump(train2, f)

with open('../data-cleaned/test.pickle', 'wb') as f:
    pickle.dump(test, f)
